...
def light_deformation(alpha=10, y_tela=1.5*10**20,  r=7*10**8, m=1.9*10**30):
    """ Функция, расчитывающая угол искривления луча света 
        Параметры функции:
            ...
            m - масса тела  
    """
    alpha = alpha*np.pi/180 #перевод угла из градусов в радианы
    plt.plot(0, y_tela, color='y', marker='o', ms=15) # солнце
    ...
    #схематичное изоброжение отрезка луча до точки искревления

    
    tetta=4*phc.G*m/(r*phc.c**2) # вычисление угла искривления (tetta)
    print(tetta*180/np.pi) # вывод чиилового значения угла искривления
    ...
    # траектория луча до преломления
    ...
    # траектория луча без переломления 
